...
async function renderPageIntoDiv(num, targetW, targetH) {
  ...
  // Get scale needed to exactly fit our target height
  const originalViewport = page.getViewport({ scale: 1.0 });
  const renderScale = targetH / originalViewport.height;
  const viewport = page.getViewport({ scale: renderScale });

  const canvas = document.createElement('canvas');
  canvas.width = viewport.width;
  canvas.height = viewport.height;
  // Use CSS to force it to fill the Turn.js bounds exactly, regardless of PDF aspect ratio
